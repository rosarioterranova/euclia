...
//Fine Livello
	var distFine = Vector3.Distance(GameObject.Find("fine").transform.position, GameObject.Find("rickCavallo").transform.position);
	
if(distFine<5 && (!finito))
	{
		Time.timeScale = 0;
		finito = true;
		completato = true;
	...
	//Gestione menu pausa
		if(Input.GetKeyDown(KeyCode.Escape) && cavalloRick.morte == false){
			if(paused ==false){
				paused = true;
				Time.timeScale = 0;
		...
		//Gestione menu morte
		if(cavalloRick.morte == true) ritentaMenu.SetActive(true);
	...
	if (menu.attivaTouch){	
			...
			for ( var i = 0 ; i < tapCount ; i++ ) {
				...
				//Bottone pausaGUI
				if(touchar.phase == TouchPhase.Began && vPausa.HitTest(touchar.position) && cavalloRick.morte == false){
					if(paused ==false){
						paused = true;
						Time.timeScale = 0;
						pausaMenu.SetActive(true);
				...
				//Bottoni menu pausa
				if (paused == true && cavalloRick.morte == false){
				...
				if (completato == true && cavalloRick.morte == false && paused == false){
					completatoMenu.SetActive(true);
					if(tadaSuonato==false){GameObject.Find("tada").audio.Play(); tadaSuonato=true;}
	...
	if (menu.attivaTasti){	
		//Bottone pausaGUI
				if(Input.GetMouseButtonDown(0) && vPausa.HitTest(Input.mousePosition) && cavalloRick.morte == false){
					if(paused ==false){
						paused = true;
						Time.timeScale = 0;
						pausaMenu.SetActive(true);
