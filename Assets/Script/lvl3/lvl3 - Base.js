...
function Update () {
		...
		//Bottone pausaGUI
				...
				if (completato == true && rick.morte == false && paused == false){
					...
					if(Input.GetMouseButtonDown(0) && prossimo.HitTest(Input.mousePosition)){
						Time.timeScale = 1;
						GameObject.Find("click").audio.Play();
						Application.LoadLevel("lvl3 Loading - Shuttle");
						completatoMenu.SetActive(false);
					}
	...
	//Fine Livello
	var distFine = Vector3.Distance(GameObject.Find("fine").transform.position, GameObject.Find("Rick Astronauta (Controller)").transform.position);
...
{
	var dist = Vector3.Distance(GameObject.Find("Rick Astronauta (Controller)").transform.position, oggetto.transform.position);
	if (cambio){
		oggetto.transform.position.x+=2*Time.deltaTime;
		if (dist<3) GameObject.Find("Rick Astronauta (Controller)").transform.position.x+=2*Time.deltaTime;
