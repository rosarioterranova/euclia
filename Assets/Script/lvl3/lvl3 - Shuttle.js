...
function Update () {
	...
	//Gestione movimenti asteroidi
	asteroidi = GameObject.FindGameObjectsWithTag("ostacolo");
	for (var j = 0; j < asteroidi.length; j++) { 
    	asteroidi[j].transform.Rotate(Vector3.forward* Time.deltaTime*50, Space.World);
	}
	
	//Gestione menu pausa
		if(Input.GetKeyDown(KeyCode.Escape) && shuttle.morte == false){
			if(paused ==false){
				paused = true;
				Time.timeScale = 0;
		...
		//Gestione menu morte
		if(shuttle.morte == true){ritentaMenu.SetActive(true); Time.timeScale = 0;}
	...
	if (menu.attivaTouch){	
			...
			for ( var i = 0 ; i < tapCount ; i++ ) {
				...
				//Bottone pausaGUI
				if(touchar.phase == TouchPhase.Began && vPausa.HitTest(touchar.position) && shuttle.morte == false){
					if(paused ==false){
						paused = true;
						Time.timeScale = 0;
						pausaMenu.SetActive(true);
				...
				//Bottoni menu pausa
				if (paused == true && shuttle.morte == false){
				...
				//Bottoni menu morte
				else if (shuttle.morte == true && paused == false){
				...
				if (completato == true && shuttle.morte == false && paused == false){
	...
	if (menu.attivaTasti){	
		//Bottone pausaGUI
				if(Input.GetMouseButtonDown(0) && vPausa.HitTest(Input.mousePosition) && shuttle.morte == false){
					if(paused ==false){
						paused = true;
						Time.timeScale = 0;
						pausaMenu.SetActive(true);
	...
	//Fine Livello
	var distFine = Vector3.Distance(GameObject.Find("fine").transform.position, GameObject.Find("shuttle").transform.position);
...
{
	var dist = Vector3.Distance(GameObject.Find("shuttle Astronauta (Controller)").transform.position, oggetto.transform.position);
	if (cambio){
		oggetto.transform.position.x+=2*Time.deltaTime;
		if (dist<3) GameObject.Find("shuttle Astronauta (Controller)").transform.position.x+=2*Time.deltaTime;
