...
function Update () {	
	...
	if(attacco == true){
		...
		GameObject.Find("Rick Astronauta (Controller)").rigidbody.AddForce(Vector3.up * 2, ForceMode.Impulse);
		...
		if(vita==0){
			...
			GameObject.Find("Rick Astronauta (Controller)").transform.position.z=1000;
			Time.timeScale = 0;
			rick.morte = true;
		}
	}
